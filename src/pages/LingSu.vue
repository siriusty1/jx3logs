<template>
  <template v-if="loaded">
    <div style="display:flex;flex-direction:column;align-items: center">
      <div style="text-align: left; font-size: 20px; font-weight: bold; width: 1400px;margin-bottom: 20px">离经易道复盘 8.0.0 Dev</div>
      <div class="infoBox">
      ...
      <el-tabs type="border-card"  class="statisticBox">
        ...
        'key':['num','numPerSec','delay','HPS','haozhenHPS','effRate'],
        'value':['数量：','每秒数量：','延迟：','HPS：','豪针HPS：','有效比例：']}">
              <template #icon>
          ...
          <div class="timeFlow">
            ...
            <div class="GCD">
              ...
              <img v-for="(item, index) in timeFlowData.timeflow_casting"
                   :src="getImageUrl(item.instant ? item.iconid + '_instant': item.iconid,'skills_logo')"
                   alt="icon"
                   class="GCD_skill_icon"
                   @mouseenter="enterImg($event,item)"
                   @mouseleave="leaveImg($event,item)"
                   :style="{
                           left:xPosition(item.start),
                           height: icon_length,
                           width: item.instant ? icon_length : Math.floor(item.duration / 1000 * 40) + 'px'
                          }">
            </div>
            <div class="noGCD">
              <div v-for="i in timeFlowData.timeScale" style="height: 100%; width: 199px;border-right: 1px solid rgba(85,85,85,0.5)"></div>
...
axios({
  ...
  if (res.data.available){
    ...
    for (let item of replay['replay']['normal']){
      if (item['skillname'] === '长针' || item['skillname'] === '提针'){
